...
- Если triggers_collapse — режим игры переходит в Consequences, **и актор условия делится «усадкой»**: наследник с `keeps_seat` остаётся в мире под тем же `id` с урезанной долей и именем своего шаблона, остальные отделяются из метрик живого родителя. `on_collapse` через путь гибели при этом **не вызывается** (docs/investigation_split_as_shrink.md §11–12; буквальная реализация измеримо ухудшала мир — там же §4)
...
3b. Мобилизация: military_size восстанавливается к ёмкости capacity = K·population^(2/3),
    но не выше неё (docs/investigation_military_source.md)
...
- Мобилизация до боя — армия успевает пополниться к столкновениям этого тика; перенос
  шага в другое место меняет измеренные числа
