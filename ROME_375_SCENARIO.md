# СЦЕНАРИЙ: ROME 375 — СЕМЬЯ АНИЦИЕВ
...
milestone_events: [
  ...
  // Семья поднялась
  {
    ...
    is_key: true,
    llm_context_shift: "Семья Анициев стала одной из значимых сил города. Их больше не игнорируют."
  },

  // Рим раскололся
  {
    id: "rome_splits",
    condition: { metric: "cohesion", actor_id: "rome", operator: "<", value: 30, duration: 20 },
    is_key: true,
    triggers_collapse: true,  // завершает сценарный период (режим Consequences); on_collapse Рима НЕ вызывается
    llm_context_shift: "Империя разделилась. Запад удержал прежнюю столицу и меньшую половину, Восток пошёл своим путём."
  },
  ...
  // Семья потеряла позиции
  {
    ...
    is_key: true,
    llm_context_shift: "Семья Анициев потеряла всё что нажила. Они снова никто."
  }
]
```
...
```
СЦЕНАРИЙ: Рим 375 — Семья Анициев
...
Семья Анициев пережила первый кризис — или не пережила.
