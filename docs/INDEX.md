...
**Триаж открытого долга — [`TRIAGE.md`](TRIAGE.md):** 44 пункта, разделённые на «требует сценариста» (20) и «чистый код» (24).

## 1a. Решения, устоявшие по ДРУГОЙ причине

Отличать от §1: там вывод **отменён**, здесь вывод **стоит**, а основание под ним
заменено. Опасность тоньше — старый довод выглядит подтверждённым, потому что
подтвердился вывод.

| где | решение | старое основание | верное |
|---|---|---|---|
| задача 25 / `investigation_dead_authored_content.md` §15.4 | не дописывать `external_pressure` спавнам | «значение вне шкалы игры» | у метрики **не было бы механизма**: константа осталась бы константой |
| `investigation_silent_authored_content.md` §17 | не расширять предикат гарда дальше | «идиомы гипотетические» | две из трёх **уже в движке**; довод — цена, а не отсутствие |
| `investigation_pressure_military_form.md` §18 | вывод о недостижимости ворот устоял | «разница несущественна» | разница **больше** запаса, но **направлена вниз** |
