...
> **Перенесено в `main` задачей 18 (сверка шапок).** Документ жил только в ветке
> `investigate/external-pressure-decay` (`42bc29a`) и был единственным разбором расследования вне
> `main`; путь из корня репозитория приведён к общему — `docs/`. Текст ниже не менялся.
>
> **Подтверждено независимо.** Задачи 10–12 переизмерили тот же вопрос другим инструментом и
> пришли к тому же: полоса вассалитета не открывается **ни разу — 0 из 216 прогонов**
> ([`investigation_vassalage_conquest.md`](investigation_vassalage_conquest.md), таблица в §3).
> Вывод «непроходимость не на оси `ep`» устоял через три захода подряд.
>
> Ещё сильнее — измерение задачи 12: `drops = 0` в 72 прогонах из 72, то есть `external_pressure`
> не отпускает **никого, ни разу, ни в одном сценарии**; она атмосфера сценария, а не сигнал
> подчинения. Оно лежит в §20.1 того же документа, но **раздел §19–24 в `main` не перенесён** —
> он остался в ветке `investigate/vassalage-submission-metric` (`04333f3`).
>
> **Оговорка о воспроизводимости.** Пробник этого расследования — `src/bin/ep_diag.rs` — **не
> коммитился**, в отличие от `vassalage_probe`, `erosion_probe` и `knowledge_gates_probe`, которые
> лежат в `src/bin/` и запускаются как есть. Числа ниже воспроизводимы только пересозданием
> инструмента по описанию метода. Это не порок вывода — он подтверждён независимо, см. выше, — но
> разница в дисциплине между этим расследованием и более поздними названа прямо: у последующих
> инструмент остаётся в репозитории именно затем, чтобы измерение можно было повторить, а не
> принять на слово.
