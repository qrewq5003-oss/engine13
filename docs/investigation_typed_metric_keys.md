...
# Стадия 2 — реализация и верификация

**Критерий приёмки выполнен: sim байт-в-байт на всех трёх сценариях.** 15 прогонов (3 сценария × [seeds 42/1/7 × 200 тиков, batch 100×50, scripted balanced 300]) — **15/15 идентичны** baseline на `db173b9`. Тесты: **70 зелёных** (65 lib + 5 conventions), новых clippy-предупреждений **0** (7 до, 7 после).

Байт-идентичность проверялась **после каждой популяции отдельно**, а не один раз в конце — иначе расхождение было бы не атрибутируемо:

| шаг | что легло | baseline |
|---|---|---|
| 1 | типы (`MetricRef`/`RelativeMetricRef`/`MetricName`), сайты адаптированы, семантика не тронута | **15/15** |
| 2 | популяция A (10 полей, S1) | **15/15** |
| 3 | популяция B (`AutoDelta`, контейнерный `Deserialize`) + `rome_375.rs` на конструкторы | **15/15** |
| 4 | популяция D (`EventConditionType`) + **`try_get`** | **15/15** |
| 5 | популяция C (`RelativeMetricRef`) + расщепление `Condition` | **15/15** |
| 6 | популяция E (`MetricName`) | **15/15** |

**Детерминизм пере-проверен, и это не формальность:** типизация сменила тип ключа у трёх `HashMap` (`effects`/`cost` → `MetricRef`, `random_events.effects` → `RelativeMetricRef`, `metrics_modifier` → `MetricName`), а значит и порядок итерации. 5 отдельных процессов × 3 сценария × сиды 42/7 — хеш стабилен во всех 30 прогонах. Порядок безопасен по той же причине, что и в #20: ключи уникальны, применение коммутативно, RNG в этих циклах не потребляется.

## Итог по сайтам (обещание постановки выполнено буквально)

| | было | стало |
|---|---|---|
| рантайм-сайтов разбора метрики | **17** | **0** |
| из них схлопнулись в прямой `.get()`/`.apply()` | — | **14** |
| остались на `resolve(target)` (цель события — бросок RNG) | — | **2** (`engine/mod.rs:458,471`) |
| остались на `try_get` (без разбора; гард «актора нет → false») | — | **1** (`eval_metric_condition`) |
| семантик разбора | **3** (`parse` / `parse_scoped` / `eval_metric_condition`) | **1** (`MetricRef::parse`, вызывается только из `Deserialize` и конструкторов) |

`parse_scoped` удалена. Её три арма разъехались ровно так, как спроектировано (§4.2): явный префикс → `MetricRef::parse` внутри `Deserialize`; голое имя + соседний `actor_id` → `resolve_at_load`, **вызывается только на загрузке**; `self.` → `RelativeMetricRef::resolve`, 2 сайта.

## Что подтвердилось на практике

**`try_get` был обязателен — и это не теория.** Проверено удалением: если популяцию D резолвить в `MetricRef` и звать `.get()` (дефолт `0.0`), baseline расходится немедленно — `rome_splits` (`rome.cohesion < 30`) начинает срабатывать после смерти самого `rome`. Гард сохранён, 15/15.

**Rust-литералы действительно вне `Deserialize`.** `rome_375.rs` потребовал 53 конструктора в `auto_deltas` и 8 в milestone-условиях. Ни один не прошёл бы через `Deserialize` — подтверждение §4.4 в коде, а не на бумаге.

**`format!`-ключ в живом контенте — нашёлся.** `milan_1477.rs` строил ключи patronage-событий через `format!("actor:{}.legitimacy", actor_id)` — ровно тот паттерн, ради устранения которого задача и существует. Переведён на `MetricRef::actor(...)`.

**Тесты сами кодировали баг.** `test_metric_ref_apply_legitimacy_clamped` и `..._military_size_min_zero` применяли дельту к `"venice.legitimacy"` / `"venice.military_size"` — дотированным ключам, то есть **писали в фантом-глобал** и проходили по неверной причине (глобал клампится в 0..100, ответ случайно совпадал). Теперь такой ключ не собирается; тесты переписаны на `actor:`-форму.

## Находка стадии 2 — **девятый сайт** (не чинится здесь)

Конвенционный тест на allowlist имён (§7 п.8) **упал на первом же прогоне** — как и гард задачи 9 в своё время.

`constantinople_1430/auto_deltas.toml` открывается пятью блоками без `actor_id`, под комментарием **«actor_id omitted = None = applies to all»**. **Такого механизма в движке нет.** Без актор-контекста голое имя — это **глобал**: пять блоков пишут `population` / `military_size` / `cohesion` / `legitimacy` / `external_pressure` в `world.global_metrics`, откуда их **никто не читает** (единственный читаемый глобал сценария — `federation_progress`).

**Актёры Константинополя не получают базового дрейфа ни по одной из этих пяти метрик — и не получали никогда.** Это тот же класс, что сайты 1–7, но его не мог поймать ни один гард: **форма ключа безупречна**, дефект — в отсутствующем скоупе.

Не чинится в этом PR по правилу ворот: включение пяти мёртвых `auto_deltas` — это изменение баланса сценария, откалиброванного (задача 6) с ними выключенными, а единственный критерий приёмки здесь — байт-в-байт. Зафиксировано в тесте явным списком `KNOWN_INERT_GLOBAL_NAMES` с пометкой «не расширять»: новый такой блок тест уронит.

**Побочно:** `milan_1477` пишет `family:family_cohesion` и `family:family_legitimacy` через patron-действия, **не имея `family_state` вообще** (`initial_family_metrics: None`) — эффекты уходят в никуда. Отдельная мелкая находка, тоже не чинится здесь.

## Изменение, видимое вне sim (не влияет на критерий приёмки)

`constantinople_1430` писал в `narrative_config.key_metrics` голый `"federation_progress"`. Канонизация даёт `"global:federation_progress"` — **та же метрика, то же значение**, но строка-ключ в промпте хрониста меняется. Sim-baseline это не трогает (нарратив не в его выводе); отмечено ради полноты.

## Что осталось как есть

- `inheritance_coefficients` — **оставлено `String`** (решение дизайна §5.G): типизация уронила бы загрузку, а починка меняет баланс. Отдельная задача.
- `initial_family_metrics` — тоже `String`, по той же причине (§5.H, ключи с `family:`-префиксом).
- `validate_interaction_rules` — по-прежнему мёртвая функция, не оживлялась (вне рамок).
- `validate_scenario` — сохранён, но похудел до ссылочной целостности (актор существует в сценарии): проверку формы забрал тип, как и предсказано §4.5.
