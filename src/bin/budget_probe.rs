...
fn price_dep_metric(
    ...
    for rule in rules {
        ...
        let delta = match rule.mode {
            ...
            // NOT FOR MERGE — mirrors the engine's `ExcessProportional`. That this arm
            // had to be hand-written here (twice) is itself the finding: this file keeps
            // two independent copies of `engine::apply_dependency_rule`.
