    ...
    let scenario = Scenario {
        ...
        narrative_config: crate::core::NarrativeConfig {
            ...
            key_metrics: vec![
                ...
                // Порядок слов перевёрнут вместе с индикатором: фронда сильна при
                // НИЗКОЙ сплочённости Неаполя. Обоснование — у индикатора ниже.
                crate::core::KeyMetric {
...
fn create_status_indicators() -> Vec<crate::core::StatusIndicator> {
    ...
    vec![
        ...
        // Фронда сильна, когда сплочённость Неаполя НИЗКА, а не высока.
        //
        // Здесь стояло наоборот: `0 → под контролем, 60 → мятеж`, то есть чем
        // сплочённее Неаполь, тем громче объявлялся мятеж. Сценарий опровергает это
        // сам, двумя своими же местами:
        //
        //   * `incite_baronial_revolt` («Подстрекать баронов Неаполя») доступно при
        //     `naples.cohesion < 45` и бьёт по ней же на `-8.0` — подстрекательство
        //     мятежа сплочённость СНИЖАЕТ;
        //   * авто-дельта `naples.cohesion` идёт `base = -0.25` и добирает ещё `-0.2`
        //     при `naples.legitimacy < 50` — это держава, расползающаяся по швам,
        //     а не бароны, сплачивающиеся против неё.
        //
        // `invert` тоже меняется: высокая сплочённость теперь благо, и полоса должна
        // краснеть на низкой. Слова совпадают с ключевой метрикой — гард
        // `key_metric_bands_agree_with_status_indicators`.
        StatusIndicator {
            label: "Баронская фронда".to_string(),
            metric: crate::core::MetricRef::literal("actor:naples.cohesion"),
            invert: false,
